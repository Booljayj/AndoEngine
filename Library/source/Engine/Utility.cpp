		...
		if (size > 0) {
			...
			do {
				*(buffer + digits) = value % 10 + '0'; //Write the lowest digit
				...
				++digits;
			} while (value > 0 && buffer != end && digits < std::numeric_limits<decltype(digits)>::max());
...
std::ostream& operator<<(std::ostream& stream, ByteSize bytes) {
	const auto WriteSmallUnit = [&stream](uint64_t size) {
		char output[6] = "B    ";
		const uint8_t digits = Utility::WriteReversedValue(size, output + 2, 3);
		const size_t outputSize = std::min<size_t>(sizeof(output)-1, digits + 2);
	...
	const auto WriteUnit = [&stream](char Suffix, uint64_t fixedPointSize) {
		...
		const uint8_t digits = Utility::WriteReversedValue(fixedPointSize / 10, output + 5, 3);
		const size_t outputSize = std::min<size_t>(sizeof(output)-1, digits + 5);
