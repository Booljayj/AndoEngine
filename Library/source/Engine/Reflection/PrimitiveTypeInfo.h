...
namespace Reflection {
	...
	struct TPrimitiveTypeInfo : public ImplementedTypeInfo<Type, PrimitiveTypeInfo> {
		TPrimitiveTypeInfo(std::string_view inName) : ImplementedTypeInfo<Type, PrimitiveTypeInfo>(Reflect<Type>::ID, inName) {}
		TYPEINFO_BUILDER_METHODS(Type);
	};
}

TYPEINFO_REFLECT(Primitive);
...
template<> struct Reflect<Type> {\
	static ::Reflection::TypeInfo const* Get();\
