...
namespace Rendering {
	void TransferQueue::Submit(std::span<VkSemaphore const> wait_semaphores, std::span<VkPipelineStageFlags const> wait_stages, std::span<VkCommandBuffer const> command_buffers, std::span<VkSemaphore const> signal_semaphores, VkFence fence) const {
	...
	void TransferQueue::Submit(std::span<VkCommandBuffer const> command_buffers, VkFence fence) const {
		VkSubmitInfo const info{
			.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO,
			.pNext = nullptr,
	...
	TransferCommandPool::TransferCommandPool(VkDevice device, TransferQueue transfer)
		: device(device)
	{
	...
	void TransferCommandPool::Reset() {
	...
	VkCommandBuffer TransferCommandPool::CreateBuffer(VkCommandBufferLevel level) {
	...
	void TransferCommandPool::DestroyBuffer(VkCommandBuffer buffer) {
		vkFreeCommandBuffers(device, pool, 1, &buffer);
	}

	std::vector<VkCommandBuffer> TransferCommandPool::CreateBuffers(uint32_t numBuffers, VkCommandBufferLevel level) {
	...
	void TransferCommandPool::DestroyBuffers(std::span<VkCommandBuffer const> buffers) {
		vkFreeCommandBuffers(device, pool, buffers.size(), buffers.data());
	}

	TransferCommandWriter::TransferCommandWriter(VkCommandBuffer commands)
		: commands(commands)
	{
	...
	TransferCommandWriter::~TransferCommandWriter() {
