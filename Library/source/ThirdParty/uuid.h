...
/* MOD : Remove support for std::span so we don't have to worry about including extra libraries or upgrading to C++20
#else
...
*/
   ...
   public:
      ...
      */
      // MOD : Remove support for std::span so we don't have to worry about including extra libraries or upgrading to C++20
      inline std::array<std::byte, 16> as_bytes() const
      {
         std::array<std::byte, 16> bytes;
         for (size_t i = 0; i < data.size(); ++i) bytes[i] = static_cast<std::byte>(data[i]);
         return bytes;
      }
