...
namespace S
{
	EntitySystem::EntitySystem( const vector<CompInfo*>& ComponentInfos )
	{
		...
		{
			ComponentInfoMap.insert( make_pair( ComponentInfo->ID, ComponentInfo ) );
		}
	}
	...
	bool EntitySystem::Initialize()
	{
		for( auto& ComponentInfoPair : ComponentInfoMap )
		{
			...
			{
				fprintf( stdout, "Fatal error initializing component manager for %s", Info->Name.c_str() );
				return false;
			}
		}
		return true;
	...
	bool EntitySystem::Deinitialize()
	{
		for( auto& ComponentInfoPair : ComponentInfoMap )
		{
			...
			{
				fprintf( stdout, "Fatal error deinitializing component manager for %s", Info->Name.c_str() );
				return false;
			}
		}
		return true;
